    ...
    private static void handlePostRegister() {
        post("/register", (req, res) -> {
            ...
            try {
                ...
                Collection<User> usersList1 = dao.loadAll(User.class);

                dao.createOrUpdate(user);

                Collection<User> usersList2 = dao.loadAll(User.class);

                //make sure email per person is unique
                if(usersList1.size() == usersList2.size())
                {
                    for(User person : usersList2)
                    {
                        if(person.getEmail().equalsIgnoreCase(user.getEmail())) {
                            existingUser = person;
                            //Set the existing data to user
